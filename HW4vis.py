...
input('We will now visualize the forward mode graph of your function. Close the graph window when you are done.  Hit Enter to continue.')

ADgraph.draw_graph2(f, G, edge_labs, pos, labs)

print('The corresponding computational table for forward mode is:')
table = ADgraph.gen_table(f)
print(table)

input('Now let\'s think about reverse mode.  We will first visualize the entire reverse graph.  (This is the graph you should save for HW4.)  Close the graph window when you are done.  Hit Enter to continue.')
ADgraph.draw_graph_rev2(f, G, edge_labs, pos, labs)

input('Let\'s step through the graph traversal for using reverse mode to find the partial derivative with respect to x.  Hit Enter to continue.')
